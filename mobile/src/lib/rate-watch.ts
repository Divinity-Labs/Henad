...
let watched: string[] = WATCHABLE

export function setWatchedCorridors(keys: string[]): void {
  watched = keys.length > 0 ? keys : WATCHABLE
}

TaskManager.defineTask(TASK, async () => {
  try {
    const { posted } = await runRateWatch(watched)
...
export async function rateWatchState(): Promise<{ on: boolean; available: boolean; lastAt: number | null }> {
  const [registered, status, seen] = await Promise.all([
